...
class Hasil_lab extends CI_Controller {
	...
	*/
	...
	public $menuID 	= 6;
	public $name 	= 'Hasil Lab';

	public function __construct(){
        parent::__construct();
		$this->load->model('M_hasil_lab');
	...
	public function ajax(){
		$get_resep = $this->M_hasil_lab->get_all();
	 ...
	 function get_list_pasien(){
		...
		$list_pasien 		= $this->M_hasil_lab->m_get_list_pasien($kd_pasien,$nama);
		// echo $this->db->last_query();
		// die();
	...
	public function insert(){
		...
		$detail 			= json_decode($detail1);
			...
			$this->M_hasil_lab->m_false_data($data_priksa,$id_daftar);
	...
	public function update(){
	   date_default_timezone_set("Asia/Jakarta");
		...
		/*================== transaction begin ======================*/
		$this->db->trans_begin();
		$this->update_group($id_hasil,$kd_pasien,$nama,$umur,$pemeriksaan,$bpjs,$dokter,$id_daftar,$date);
		$this->M_hasil_lab->delete_detail($id_hasil);
		$this->insert_detail($id_hasil,$pemeriksaan,$kd_pasien,$detail,$date);
	...
	function update_group($id_hasil,$kd_pasien,$nama,$umur,$pemeriksaan,$bpjs,$dokter,$id_daftar,$date){
		$data_group = array(
						'KODE_PASIEN' 		=> $kd_pasien,
						'NAMA_LENGKAP' 		=> $nama,
						'UMUR'				=> $umur,
		...
		$this->M_hasil_lab-> m_update_group($data_group,$id_hasil);
	}
	...
	function suspend_data(){
		...
		$this->db->trans_begin();
		$this->M_hasil_lab->m_suspend_data($data_suspend,$no_transaction);
		    ...
		    $html .= "<table width='100%' border='0' cellpadding='2' cellspacing='0' class='content-table'>"
	        	...
	        	foreach($detail_rs as $row){
	        		$html .= "<tr class='body-report'>"
	        			...
	        			."<td align='center'>".substr($row->PRIKSA, 0,35)."</td>"
