...
class Pem_gizi_anak extends CI_Controller {
	...
	*/
	...
	public $menuID 	= 11;
	public $name 	= 'Pemeriksaan Gizi Anak';

	public function __construct(){
        parent::__construct();
		$this->load->model('M_pem_gizi_anak');
	...
	public function ajax(){
		$get_pemeriksaan = $this->M_pem_gizi_anak->get_all();
	...
	public function index(){
		...
		$this->load->view('pemeriksaan_gizi/data_pemeriksaan_gizi_anak',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_pemgizi->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['TransNumber'] 	= $this->M_pem_gizi_anak->create_no_transaction();	
		$this->load->view('pemeriksaan_gizi/header1');
		$this->load->view('pemeriksaan_gizi/new_pemeriksaan_gizi_anak',$data);
	...
	public function preview($id_pemeriksaan){
		...
		$data['menu_link'] 	= $this->M_menu_pemgizi->get_menu($this->menuID);	
		$data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_pem_gizi_anak->m_get_group($id_pemeriksaan);
		$this->load->view('pemeriksaan_gizi/header1');
		$this->load->view('pemeriksaan_gizi/prev_pem_gizi_anak',$data);
		$this->load->view('pemeriksaan_gizi/footer');
	}
	...
	function get_list_diagnosa(){
		...
		$list_diagnosa 		= $this->M_pem_gizi_anak->m_get_list_diagnosa($id_diagnosa,$des_icd);
		// echo $this->db->last_query();
		// die();
    ...
    $pasien = $this->M_pem_gizi_anak->kd_pasien($kd);
	...
	public function insert(){
		...
		$id_pem 		= $this->M_pem_gizi_anak->create_no_transaction();	
		...
		$status     		= $this->input->post('status');
			...
			$this->insert_data($id_pem,$kd_pasien,$nama,$tt_lahir,$alamat,$bpjs,$no,$po,$akd,$diag,$des,$bbl,$pbl,$uk,$kelahiran,$tb,$bb,$imt,$bio,$fk,$id_berobat);
	...
	function insert_data($id_pem,$kd_pasien,$nama,$tt_lahir,$alamat,$bpjs,$no,$po,$akd,$diag,$des,$bbl,$pbl,$uk,$kelahiran,$tb,$bb,$imt,$bio,$fk,$id_berobat){
		$data_priksa = array(
						'ID_PG_ANAK' 		=> $id_pem,
		...
		$this->M_pem_gizi_anak->m_insert_data($data_priksa);
	}
	...
	public function update(){
		...
		$po 			= $this->input->post('po');
		$akd 			= $this->input->post('akd');
		$id_diag 			= $this->input->post('diagnosa');
		$des_icd 			= $this->input->post('deskripsi');
		$bbl 			= $this->input->post('bbl');
		$pbl 			= $this->input->post('pbl');
		$uk 			= $this->input->post('uk');
		$kelahiran 			= $this->input->post('kelahiran');
		...
		$this->update_data($id_pem,$kd_pasien,$nama,$tt_lahir,$alamat,$bpjs,$no,$po,$akd,$id_diag,$des_icd,$bbl,$pbl,$uk,$kelahiran,$tb,$bb,$imt,$bio,$fk,$id_berobat,$status);
	...
	function update_data($id_pem,$kd_pasien,$nama,$tt_lahir,$alamat,$bpjs,$no,$po,$akd,$id_diag,$des_icd,$bbl,$pbl,$uk,$kelahiran,$tb,$bb,$imt,$bio,$fk,$id_berobat,$status){
		...
		$this->M_pem_gizi_anak->m_update_group($data_pemeriksaan,$id_pem);
	}
