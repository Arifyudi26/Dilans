...
class Pem_tarif_lab extends CI_Controller {

	public $menuID 	= 19;
	public $name 	= 'Pemeriksaan & Tarif Lab';

	public function __construct(){
        parent::__construct();
		$this->load->model('M_pem_tarif_lab');
	...
	public function ajax(){
		$get = $this->M_pem_tarif_lab->get_all();
	...
	public function index(){
		...
		$this->load->view('m_bersalin/v_pem_tarif_lab',$data);
	}
	...
	public function insert(){
		...
		if($status == 1){
			...
			$id_npl 	= $this->M_pem_tarif_lab->create_no_transaction();

			$data_insert = array(
							'ID_DT_LAB'  		=> $id_npl,
							'NAMA_PEMERIKSAAN'	=> $npl,
							'TARIF' 			=> $tarif,
							'TGL_BERLAKU'		=> $tgl,
							'KETERANGAN' 		=> $ket,
							'status' 			=> 0,
			...
			$this->M_pem_tarif_lab->save_data($data_insert);
			...
			$this->M_pem_tarif_lab->update_data($data,$code);
		...
		public function print_data($start_date='',$end_date='',$id_dt_lab='',$dt_lab=''){
		$id_dt_lab		= str_replace('__', '/', $id_dt_lab);
		$start_date 		= str_replace('__', '-', $start_date);
		$end_date 			= str_replace('__', '-', $end_date);

		$dt_lab				= str_replace('__', ',', $dt_lab);
		$dt_lab				= str_replace('%20', ' ', $dt_lab);
        ...
        $data_list 		= $this->M_pem_tarif_lab->get_list_lab($start_date,$end_date,$id_dt_lab,$dt_lab);
        ...
        for($x=0;$x<$total_page;$x++){
		    ...
		    $html .= "<table width='100%' cellpadding='5' cellspacing='0'>"
		    		."<tr>"
		    		."<td colspan='7' align='center'><h3>Laporan Pemeriksaan Tarif Lab</h3>"
		    		."</td>"
		    		."</tr>"
		    		."<tr>";
		    ...
		    if($total_baris < $max_baris){
		    	$body 	= "<tr>"
				    		...
				    		."<td class='bdr-right bdr-top bdr-bottom'>Id Pem DT Lab</td>"
							."<td class='bdr-right bdr-top bdr-bottom'>Nama Pemeriksaan</td>"
							."<td class='bdr-right bdr-top bdr-bottom' align='center'>Tarif</td>"
							."<td class='bdr-right bdr-top bdr-bottom' align='center'>Tgl Berlaku</td>"
				 ...
				 for($i=$min_to_baris;$i<$max_to_baris;$i++){
				 	$body 	.= "<tr>"
		    				...
		    				."<td>".$list[$i]['ID_DT_LAB']."</td>"
		    				."<td>".$list[$i]['NAMA_PEMERIKSAAN']."</td>"
		    				."<td class='bdr-center'>".$list[$i]['TARIF']."</td>"
							."<td class='bdr-center'>".$list[$i]['TGL_BERLAKU']."</td>"
