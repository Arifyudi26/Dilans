...
class Perawatan_bayi extends CI_Controller {

	public $menuID 	= 7;
	public $name 	= 'Perawatan Ibu';

	public function __construct(){
        parent::__construct();
		$this->load->model('M_perawatan_bayi');
	...
	public function ajax(){
		$get = $this->M_perawatan_bayi->get_all();
	...
	public function index(){
		...
		$this->load->view('m_bersalin/v_perawatan_bayi',$data);
	}

	public function insert(){
		$perbayi 	= $this->input->post('perbayi');
		...
		if($status == 1){
			...
			$id_perbayi 	= $this->M_perawatan_bayi->create_no_transaction();

			$data_insert = array(
							'ID_PERAWATAN_BAYI' 	=> $id_perbayi,
							'PERAWATAN_BAYI'		=> $perbayi,
							'TARIF' 	=> $tarif,
							'KETERANGAN' 	=> $ket,
							'status' 		=> 0,
							'ID_USER' 		=> 1);

			$this->M_perawatan_bayi->save_data($data_insert);
			...
			$this->M_perawatan_bayi->update_data($data,$code);
		...
		public function print_data($start_date='',$end_date='',$id_bayi='',$perbayi=''){
		$id_bayi		= str_replace('__', '/', $id_bayi);
		$start_date 		= str_replace('__', '-', $start_date);
		$end_date 			= str_replace('__', '-', $end_date);

		$perbayi				= str_replace('__', ',', $perbayi);
		$perbayi				= str_replace('%20', ' ', $perbayi);
        ...
        $data_list 		= $this->M_perawatan_bayi->get_list_bayi($start_date,$end_date,$id_bayi,$perbayi);
        ...
        for($x=0;$x<$total_page;$x++){
		    ...
		    $html .= "<table width='100%' cellpadding='5' cellspacing='0'>"
		    		."<tr>"
		    		."<td colspan='7' align='center'><h3>Laporan Perawatan Bayi</h3>"
		    		."</td>"
		    		."</tr>"
		    		."<tr>";
		    ...
		    if($total_baris < $max_baris){
		    	$body 	= "<tr>"
				    		...
				    		."<td class='bdr-right bdr-top bdr-bottom'>Id Per Bayi</td>"
							."<td class='bdr-right bdr-top bdr-bottom'>Perawatan Bayi</td>"
				 ...
				 for($i=$min_to_baris;$i<$max_to_baris;$i++){
				 	$body 	.= "<tr>"
		    				...
		    				."<td>".$list[$i]['ID_PERAWATAN_BAYI']."</td>"
		    				."<td>".$list[$i]['PERAWATAN_BAYI']."</td>"
	    		...
	    		$mpdf->Output('Data_Perbayi.pdf', 'I');
