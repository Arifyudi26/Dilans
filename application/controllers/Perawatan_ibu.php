...
class Perawatan_ibu extends CI_Controller {
	...
	public function __construct(){
        parent::__construct();
		$this->load->model('M_perawatan_ibu');
	...
	public function ajax(){
		$get = $this->M_perawatan_ibu->get_all();
	...
	public function index(){
		...
		$this->load->view('m_bersalin/v_perawatan_ibu',$data);
	}

	public function insert(){
		$peribu 	= $this->input->post('peribu');
		...
		if($status == 1){
			...
			$id_peribu 	= $this->M_perawatan_ibu->create_no_transaction();

			$data_insert = array(
							'ID_PERAWATAN_IBU' 	=> $id_peribu,
							'PERAWATAN_IBU'		=> $peribu,
							'TARIF' 	=> $tarif,
							'KETERANGAN' 	=> $ket,
							'status' 		=> 0,
							'ID_USER' 		=> 1);

			$this->M_perawatan_ibu->save_data($data_insert);
			...
			$this->M_perawatan_ibu->update_data($data,$code);
	...
	public function print_data($start_date='',$end_date='',$id_ibu='',$peribu=''){
		$id_ibu		= str_replace('__', '/', $id_ibu);
		$start_date 		= str_replace('__', '-', $start_date);
		$end_date 			= str_replace('__', '-', $end_date);

		$peribu				= str_replace('__', ',', $peribu);
		$peribu				= str_replace('%20', ' ', $peribu);
        ...
        $data_list 		= $this->M_perawatan_ibu->get_list_ibu($start_date,$end_date,$id_ibu,$peribu);
        ...
        for($x=0;$x<$total_page;$x++){
		    ...
		    $html .= "<table width='100%' cellpadding='5' cellspacing='0'>"
		    		."<tr>"
		    		."<td colspan='7' align='center'><h3>Laporan Perawatan Ibu</h3>"
		    		."</td>"
		    		."</tr>"
		    		."<tr>";
		    ...
		    if($total_baris < $max_baris){
		    	$body 	= "<tr>"
				    		...
				    		."<td class='bdr-right bdr-top bdr-bottom'>Id Per Ibu</td>"
							."<td class='bdr-right bdr-top bdr-bottom'>Perawatan Ibu</td>"
				 ...
				 for($i=$min_to_baris;$i<$max_to_baris;$i++){
				 	$body 	.= "<tr>"
		    				...
		    				."<td>".$list[$i]['ID_PERAWATAN_IBU']."</td>"
		    				."<td>".$list[$i]['PERAWATAN_IBU']."</td>"
