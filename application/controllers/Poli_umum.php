...
class Poli_umum extends CI_Controller {
	...
	*/
	...
	public $menuID 	= 11;
	public $name 	= 'Poli Umum';

	public function __construct(){
        parent::__construct();
		$this->load->model('M_poli_umum');
	...
	public function ajax(){
		$get_tindakan = $this->M_poli_umum->get_all();
	...
	function get_list_dokter(){
		$list_dokter = $this->M_poli_umum->m_get_list_dokter();
	...
	public function index(){
		...
		$this->load->view('medrec/data_poli_umum',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_poli_umum->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['TransNumber'] 	= $this->M_poli_umum->create_no_transaction();	
		$this->load->view('medrec/header1');
		$this->load->view('medrec/new_data_poli_umum',$data);
		$this->load->view('medrec/footer1');
	}

	public function preview($id_poli_umum){

		$data['menu_link'] 	= $this->M_menu_poli_umum->get_menu($this->menuID);	
		$data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_poli_umum->m_get_group($id_poli_umum);
		$this->load->view('medrec/header1');
		$this->load->view('medrec/prev_poli_umum',$data);
		$this->load->view('medrec/footer1');
	}
	...
	function get_list_diagnosa(){
		...
		$list_diagnosa 		= $this->M_poli_umum->m_get_list_diagnosa($id_diagnosa,$des_icd);
		// echo $this->db->last_query();
		// die();
   ...
   function get_list_pasien(){
		...
		$list_pasien 		= $this->M_poli_umum->m_get_list_pasien($kd_pasien,$nama);
		// echo $this->db->last_query();
		// die();
	...
	public function insert(){
		...
		$id_poli_umum 		= $this->M_poli_umum->create_no_transaction();	
		...
		$status		  		= $this->input->post('status');
          ...
          $data_priksa = array('trans_status'=>1);
			...
			$this->insert_data($id_poli_umum,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$keluhan,$tb,$bb,$ss,$ds,$rr,$hr,$pd,$diagnosa,$deskripsi,$id_dokter,$id_pemeriksaan,$status,$date);

			$this->M_poli_umum->m_false_data($data_priksa,$id_pemeriksaan);
	...
	function insert_data($id_poli_umum,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$keluhan,$tb,$bb,$ss,$ds,$rr,$hr,$pd,$diagnosa,$deskripsi,$id_dokter,$id_pemeriksaan,$status,$date){
		$data_umum = array(
		...
		$this->M_poli_umum->m_insert_data($data_umum);
	}
	...
	public function update(){
		...
		$this->update_data($id_poli_umum,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$keluhan,$tb,$bb,$ss,$ds,$rr,$hr,$pd,$diagnosa,$deskripsi,$id_dokter,$id_pemeriksaan,$status,$date);
	...
	function update_data($id_poli_umum,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$keluhan,$tb,$bb,$ss,$ds,$rr,$hr,$pd,$diagnosa,$deskripsi,$id_dokter,$id_pemeriksaan,$status,$date){
		$data_umum = array(
						'KODE_PASIEN' 			=> $kd_pasien,
						'NAMA_LENGKAP' 			=> $nama,
						'JENIS_KELAMIN'  	=> $jenis,
		...
		$this->M_poli_umum->m_update_data($data_umum,$id_poli_umum);
	}
    ...
    function suspend_data(){
		...
		$this->M_poli_umum->m_suspend_data($data_suspend,$no_transaction);
