...
class Resep_obat_gigi extends CI_Controller {
	...
	*/
	...
	public function __construct(){
        parent::__construct();
		$this->load->model('M_resobat_gigi');
	...
	public function ajax(){
		$get_resep = $this->M_resobat_gigi->get_all();
    ...
    $pasien = $this->M_resobat_gigi->kd_pasien($kd);
	...
	function get_list_dokter(){
		$list_dokter = $this->M_resobat_gigi->m_get_list_dokter();
	...
	public function index(){
		...
		$this->load->view('poli_gigi/data_resep',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_poli_gigi->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['gigi'] 	= $this->M_resobat_gigi->create_no_transaction();	
		$this->load->view('poli_gigi/header1');
		$this->load->view('poli_gigi/new_data_resep',$data);
		$this->load->view('poli_gigi/footer1');
	}

	public function preview($id_resob_gigi){

		$data['menu_link'] 	= $this->M_menu_poli_gigi->get_menu($this->menuID);	
	    $data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_resobat_gigi->m_get_group($id_resob_gigi);
		$this->load->view('poli_gigi/header1');
		$this->load->view('poli_gigi/prev_resep_obat',$data);
		$this->load->view('poli_gigi/footer1');
	}
	...
	public function insert(){
		...
		$id_poli_gigi			= $this->input->post('id_poli');
		$detail1 			= $this->input->post('detail');
		$detail 			= json_decode($detail1);

			$this->db->trans_begin();

			$this->insert_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_gigi);
	...
	function insert_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_gigi){
		$data_resep = array(
						'ID_RESOB_GIGI' 			=> $id_resep,
						...
						'ID_DOKTER'			=> $dokter,
						'ID_POLI_GIGI' 		=> $id_poli_gigi,
						'status' 	  		=> 0,
						'ID_USER' 			=> 5);

		$this->M_resobat_gigi->m_insert_resep($data_resep);
	}
	...
	function insert_detail($id_resep,$kd_pasien,$nama,$detail,$date){
		...
		foreach($detail as $row){
			...
			$this->M_resobat_gigi->m_insert_detail($data_detail);
	...
	public function update(){
		...
		$this->update_group($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_gigi);
		$this->M_resobat_gigi->delete_detail($id_resep);
	    $this->insert_detail($id_resep,$kd_pasien,$nama,$detail,$date);
	...
	function update_group($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_gigi){
		$data_group = array(
						'KODE_PASIEN' 			=> $kd_pasien,
						'NAMA_LENGKAP' 			=> $nama,
						'UMUR'				=> $umur,
		...
		$this->M_resobat_gigi->m_update_group($data_group,$id_resep);
	}
	...
	function get_list_obat(){
		...
		$list_obat 		= $this->M_resobat_gigi->m_get_list_obat($id_obat,$nama_obat,$satuan);
		// echo $this->db->last_query();
		// die();
	...
	function get_detail_resep($id_resob_gigi){
		$list_detail = $this->M_resobat_gigi->m_get_detail($id_resob_gigi);
		echo json_encode($list_detail);
	}
	
	function print_preview($id_resob_gigi){

		include(APPPATH . 'libraries/mpdf/mpdf.php');
         $mpdf = new mPDF('utf-8', A4, 10, '', 5,5,5,5,'','','P');
        $mpdf->SetFooter('{PAGENO}');

        $group_rs		= $this->M_resobat_gigi->get_group_resep($id_resob_gigi);

        $detail_rs 		= $this->M_resobat_gigi->get_detail_resep($id_resob_gigi);
		
        $total_baris 	= $this->M_resobat_gigi->count_detail_resep($id_resob_gigi); 
        $max_baris 		= 10;
		    			...
		    			."<td valign='top' width='40%'>"
		    				."<table width='100%'>"
		    					."<tr>"
			    					...
			    					."<td width='5%'>:</td>"
			    					."<td width='55%'>$group_rs->ID_RESOB_GIGI</td>"
		    					."</tr>"
		    					."<tr>"
