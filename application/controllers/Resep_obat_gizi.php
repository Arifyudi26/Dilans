...
class Resep_obat_gizi extends CI_Controller {
	...
	*/
	...
	public function __construct(){
        parent::__construct();
		$this->load->model('M_resobat_gizi');
	...
	public function ajax(){
		$get_resep = $this->M_resobat_gizi->get_all();
    ...
    $pasien = $this->M_resobat_gizi->kd_pasien($kd);
	...
	function get_list_dokter(){
		$list_dokter = $this->M_resobat_gizi->m_get_list_dokter();
	...
	public function index(){
		...
		$this->load->view('poli_gizi/data_resep',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_poli_gizi->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['gigi'] 	= $this->M_resobat_gizi->create_no_transaction();	
		$this->load->view('poli_gizi/header1');
		$this->load->view('poli_gizi/new_data_resep',$data);
		$this->load->view('poli_gizi/footer');
	}

	public function preview($id_resob_gizi){

		$data['menu_link'] 	= $this->M_menu_poli_gizi->get_menu($this->menuID);	
	    $data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_resobat_gizi->m_get_group($id_resob_gizi);
		$this->load->view('poli_gizi/header1');
		$this->load->view('poli_gizi/prev_resep_obat',$data);
		$this->load->view('poli_gizi/footer');
	}
	...
	function insert_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli){
		...
		$this->M_resobat_gizi->m_insert_resep($data_resep);
	}
	...
	function insert_detail($id_resep,$kd_pasien,$nama,$detail,$date){
		...
		foreach($detail as $row){
			...
			$this->M_resobat_gizi->m_insert_detail($data_detail);
	...
	public function update(){
		...
		$this->M_resobat_gizi->delete_detail($id_resep);
	    $this->insert_detail($id_resep,$kd_pasien,$nama,$detail,$date);
	...
	function update_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli){
		...
		$this->M_resobat_gizi->m_update_resep($data_resep,$id_resep);
	}
	...
	function get_list_diagnosa(){
		...
		$list_diagnosa 		= $this->M_resobat_gizi->m_get_list_diagnosa($id_diagnosa,$des_icd);
		// echo $this->db->last_query();
		// die();
	...
	function get_list_obat(){
		...
		$list_obat 		= $this->M_resobat_gizi->m_get_list_obat($id_obat,$nama_obat,$satuan);
		// echo $this->db->last_query();
		// die();
		echo json_encode($list_obat);
	}

	function get_detail_resep($id_resob_gizi){
		$list_detail = $this->M_resobat_gizi->m_get_detail($id_resob_gizi);
		echo json_encode($list_detail);
	}


	function print_preview($id_resob_gizi){

		include(APPPATH . 'libraries/mpdf/mpdf.php');
         $mpdf = new mPDF('utf-8', A4, 10, '', 5,5,5,5,'','','P');
        $mpdf->SetFooter('{PAGENO}');

        $group_rs		= $this->M_resobat_gizi->get_group_resep($id_resob_gizi);

        $detail_rs 		= $this->M_resobat_gizi->get_detail_resep($id_resob_gizi);
		
        $total_baris 	= $this->M_resobat_gizi->count_detail_resep($id_resob_gizi); 
        $max_baris 		= 10;
		    			...
		    			."<td valign='top' width='40%'>"
		    				."<table width='100%'>"
		    					."<tr>"
			    					...
			    					."<td width='5%'>:</td>"
			    					."<td width='55%'>$group_rs->ID_RESOB_GIZI</td>"
		    					."</tr>"
		    					."<tr>"
