...
class Resep_obat_lansia extends CI_Controller {
	...
	*/
	...
	public function __construct(){
        parent::__construct();
		$this->load->model('M_resobat_lansia');
	...
	public function ajax(){
		$get_resep = $this->M_resobat_lansia->get_all();
    ...
    $pasien = $this->M_resobat_lansia->kd_pasien($kd);
    ...
    if( ! empty($pasien)){ // Jika data siswa ada/ditemukan
      ...
      $callback = array(
		...
		'umur' => $pasien->UMUR,
		'diagnosa' => $pasien->DESKRIPSI_ICD,
		'id_poli' => $pasien->ID_POLI_LANSIA,
	...
	function get_list_dokter(){
		$list_dokter = $this->M_resobat_lansia->m_get_list_dokter();
	...
	public function index(){
		...
		$this->load->view('poli_lansia/data_resep',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_poli_lansia->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['gigi'] 	= $this->M_resobat_lansia->create_no_transaction();	
		$this->load->view('poli_lansia/header1');
		$this->load->view('poli_lansia/new_data_resep',$data);
		$this->load->view('poli_lansia/footer');
	}

	public function preview($id_resob_lansia){

		$data['menu_link'] 	= $this->M_menu_poli_lansia->get_menu($this->menuID);	
	    $data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_resobat_lansia->m_get_group($id_resob_lansia);
		$this->load->view('poli_lansia/header1');
		$this->load->view('poli_lansia/prev_resep_obat',$data);
		$this->load->view('poli_lansia/footer');
	}
	...
	public function insert(){
		...
		$id_poli_lansia			= $this->input->post('id_poli');
		$detail1 			= $this->input->post('detail');
		$detail 			= json_decode($detail1);

			$this->db->trans_begin();

			$this->insert_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_lansia);
	...
	function insert_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_lansia){
		$data_resep = array(
						'ID_RESOB_LANSIA' 			=> $id_resep,
						...
						'ID_DOKTER'			=> $dokter,
						'ID_POLI_LANSIA' 		=> $id_poli_lansia,
						'status' 	  		=> 0,
						'ID_USER' 			=> 10);

		$this->M_resobat_lansia->m_insert_resep($data_resep);
	}
	...
	function insert_detail($id_resep,$kd_pasien,$nama,$detail,$date){
		...
		foreach($detail as $row){
			...
			$this->M_resobat_lansia->m_insert_detail($data_detail);
	...
	public function update(){
		...
		$this->update_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_lansia);
		$this->M_resobat_lansia->delete_detail($id_resep);
		$this->insert_detail($id_resep,$kd_pasien,$nama,$detail,$date);
	...
	function update_resep($id_resep,$kd_pasien,$nama,$umur,$diagnosa,$total_qty,$dokter,$id_poli_lansia){
		$data_resep = array(
						'KODE_PASIEN' 			=> $kd_pasien,
						'NAMA_LENGKAP' 			=> $nama,
						'UMUR'				=> $umur,
		...
		$this->M_resobat_lansia->m_update_resep($data_resep,$id_resep);
	}
	...
	function get_list_obat(){
		...
		$list_obat 		= $this->M_resobat_lansia->m_get_list_obat($id_obat,$nama_obat,$satuan);
		// echo $this->db->last_query();
		// die();
	...
	function get_detail_resep($id_resob_lansia){
		$list_detail = $this->M_resobat_lansia->m_get_detail($id_resob_lansia);
		echo json_encode($list_detail);
	}

	function get_list_warehouse(){
		$list_warehouse = $this->m_purchase_request->m_get_list_warehouse();

		echo json_encode($list_warehouse);
	}

	function print_preview($id_resob_lansia){

		include(APPPATH . 'libraries/mpdf/mpdf.php');
         $mpdf = new mPDF('utf-8', A4, 10, '', 5,5,5,5,'','','P');
        $mpdf->SetFooter('{PAGENO}');

        $group_rs		= $this->M_resobat_lansia->get_group_resep($id_resob_lansia);

        $detail_rs 		= $this->M_resobat_lansia->get_detail_resep($id_resob_lansia);
		
        $total_baris 	= $this->M_resobat_lansia->count_detail_resep($id_resob_lansia); 
        $max_baris 		= 10;
		    			...
		    			."<td valign='top' width='40%'>"
		    				."<table width='100%'>"
		    					."<tr>"
			    					...
			    					."<td width='5%'>:</td>"
			    					."<td width='55%'>$group_rs->ID_RESOB_LANSIA</td>"
		    					."</tr>"
		    					."<tr>"
