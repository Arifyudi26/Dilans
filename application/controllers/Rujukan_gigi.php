...
class Rujukan_gigi extends CI_Controller {
	...
	*/
	...
	public function __construct(){
        parent::__construct();
		$this->load->model('M_rujukan_gigi');
	...
	public function ajax(){
		$get_rujukan = $this->M_rujukan_gigi->get_all();
	...
	function get_list_dokter(){
		$list_dokter = $this->M_rujukan_gigi->m_get_list_dokter();
		echo json_encode($list_dokter);
	}
	
	function get_list_poli(){
		$list_poli = $this->M_rujukan_gigi->m_get_list_poli();
	...
	public function index(){
		...
		$this->load->view('poli_gigi/data_rujukan',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_poli_gigi->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['TransNumber'] 	= $this->M_rujukan_gigi->create_no_transaction();	
		$this->load->view('poli_gigi/header1');
		$this->load->view('poli_gigi/new_data_rujukan',$data);
		$this->load->view('poli_gigi/footer1');
	}

	public function preview($id_rujukan){

		$data['menu_link'] 	= $this->M_menu_poli_gigi->get_menu($this->menuID);	
		$data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_rujukan_gigi->m_get_group($id_rujukan);
		$this->load->view('poli_gigi/header1');
		$this->load->view('poli_gigi/prev_data_rujukan',$data);
		$this->load->view('poli_gigi/footer1');
	}
	...
	function get_list_diagnosa(){
		...
		$list_diagnosa 		= $this->M_rujukan_gigi->m_get_list_diagnosa($id_diagnosa,$des_icd);
		// echo $this->db->last_query();
		// die();
    ...
    $pasien = $this->M_rujukan_gigi->kd_pasien($kd);
    ...
    if( ! empty($pasien)){ // Jika data siswa ada/ditemukan
      ...
      $callback = array(
		...
		'bpjs' => $pasien->NO_BPJS,
		'id_poli_gigi' => $pasien->ID_POLI_GIGI,
	...
	public function insert(){
		...
		$id_rujukan 		= $this->M_rujukan_gigi->create_no_transaction();	
		...
		$poli_gigi          = $this->input->post('id_poli_gigi');
		$status		  		= $this->input->post('status');
			...
			$this->insert_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poli_rujukan,$pemeriksaan,$diagnosa,$des,$terapi,$poli_gigi,$status,$date);
	...
	function insert_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poli_rujukan,$pemeriksaan,$diagnosa,$des,$terapi,$poli_gigi,$status,$date){
		$data_rujukan = array(
						'ID_RUJUKAN_GIGI' 	=> $id_rujukan,
						...
						'ICD_SEMENTARA'        => $des,
						'TERAPI'            => $terapi,
						'ID_POLI_GIGI'      => $poli_gigi,
						'Status' 		    => 0,
						'ID_USER' 			=> 5);

		$this->M_rujukan_gigi->m_insert_data($data_rujukan);
	}
	...
	public function update(){
		...
		$id_rujukan 		= $this->input->post('code');	
		$kd_pasien      	= $this->input->post('kd_pasien');
		$nama     	 		= $this->input->post('nm_lengkap');
		$jenis			 	= $this->input->post('jenis');
		$umur				= $this->input->post('umur');
		...
		$des 		  		= $this->input->post('DESKRIPSI_ICD');
		$terapi 			= $this->input->post('terapi');
		$id_polgigi 		  	= $this->input->post('id_poli_gigi');
     	$status		  		= $this->input->post('status');
		/*================== transaction begin ======================*/
		$this->db->trans_begin();

		$this->update_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poljuk,$pemeriksaan,$id_diag,$des,$terapi,$id_polgigi,$date,$status);
	...
	function update_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poljuk,$pemeriksaan,$id_diag,$des,$terapi,$id_polgigi,$date,$status){
		$data_rujukan = array(
						'KODE_PASIEN' 			=> $kd_pasien,
						'NAMA_LENGKAP' 			=> $nama,
						'JENIS_KELAMIN'  	=> $jenis,
						...
						'ICD_SEMENTARA'        => $des,
						'TERAPI'            => $terapi,
						'ID_POLI_GIGI'      => $id_polgigi,
						'Status' 		    => 0,
						'ID_USER' 			=> 5);

		$this->M_rujukan_gigi->m_update_data($data_rujukan,$id_rujukan);
	}
