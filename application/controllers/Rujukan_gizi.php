...
class Rujukan_gizi extends CI_Controller {
	...
	*/
	...
	public function __construct(){
        parent::__construct();
		$this->load->model('M_rujukan_gizi');
	...
	public function ajax(){
		$get_rujukan = $this->M_rujukan_gizi->get_all();
	...
	function get_list_dokter(){
		$list_dokter = $this->M_rujukan_gizi->m_get_list_dokter();
		echo json_encode($list_dokter);
	}
	
	function get_list_poli(){
		$list_poli = $this->M_rujukan_gizi->m_get_list_poli();
	...
	public function index(){
		...
		$this->load->view('poli_gizi/data_rujukan',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_poli_gizi->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['TransNumber'] 	= $this->M_rujukan_gizi->create_no_transaction();	
		$this->load->view('poli_gizi/header1');
		$this->load->view('poli_gizi/new_data_rujukan',$data);
	...
	public function preview($id_rujukan){
		...
		$data['menu_link'] 	= $this->M_menu_poli_gizi->get_menu($this->menuID);	
		$data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_rujukan_gizi->m_get_group($id_rujukan);
		$this->load->view('poli_gizi/header1');
		$this->load->view('poli_gizi/prev_data_rujukan',$data);
		$this->load->view('poli_gizi/footer');
	}
	...
	function get_list_diagnosa(){
		...
		$list_diagnosa 		= $this->M_rujukan_gizi->m_get_list_diagnosa($id_diagnosa,$des_icd);
		// echo $this->db->last_query();
		// die();
    ...
    $pasien = $this->M_rujukan_gizi->kd_pasien($kd);
	...
	public function insert(){
		...
		$id_rujukan 		= $this->M_rujukan_gizi->create_no_transaction();	
		...
		$status		  		= $this->input->post('status');
			...
			$this->insert_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poli_rujukan,$pemeriksaan,$diagnosa,$des,$terapi,$id_berobat,$status,$date);
	...
	function insert_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poli_rujukan,$pemeriksaan,$diagnosa,$des,$terapi,$id_berobat,$status,$date){
		$data_rujukan = array(
						'ID_RUJUKAN_GIZI' 	=> $id_rujukan,
		...
		$this->M_rujukan_gizi->m_insert_data($data_rujukan);
	}
	...
	public function update(){
		...
		$this->update_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poljuk,$pemeriksaan,$id_diag,$des,$terapi,$id_berobat,$date,$status);
	...
	function update_data($id_rujukan,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poljuk,$pemeriksaan,$id_diag,$des,$terapi,$id_berobat,$date,$status){
		$data_rujukan = array(
						'KODE_PASIEN' 			=> $kd_pasien,
						'NAMA_LENGKAP' 			=> $nama,
						'JENIS_KELAMIN'  	=> $jenis,
		...
		$this->M_rujukan_gizi->m_update_data($data_rujukan,$id_rujukan);
	}
