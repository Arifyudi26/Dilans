...
class Umb_rujukan_gigi extends CI_Controller {
	...
	*/
	...
	public function ajax(){
		$get_umb = $this->M_umb_gigi->get_all();
	...
	function get_list_dokter(){
		$list_dokter = $this->M_umb_gigi->m_get_list_dokter();
		echo json_encode($list_dokter);
	}
	
	function get_list_poli(){
		$list_poli = $this->M_umb_gigi->m_get_list_poli();
	...
	public function index(){
		...
		$this->load->view('poli_gigi/data_umb_rujukan',$data);
	}

	public function add(){
		$data['menu_link'] = $this->M_menu_poli_gigi->get_menu($this->menuID);
		$data['date_now'] 	= date('Y-m-d');
		$data['TransNumber'] 	= $this->M_umb_gigi->create_no_transaction();	
		$this->load->view('poli_gigi/header1');
		$this->load->view('poli_gigi/new_umb_rujukan',$data);
	...
	public function preview($id_umb){
		...
		$data['menu_link'] 	= $this->M_menu_poli_gigi->get_menu($this->menuID);	
		$data['date_now'] 	= date('Y-m-d');
		$data['group'] 		= $this->M_umb_gigi->m_get_group($id_umb);
		$this->load->view('poli_gigi/header1');
		$this->load->view('poli_gigi/prev_umb_rujukan',$data);
		$this->load->view('poli_gigi/footer1');
	}
    ...
    $pasien = $this->M_umb_gigi->kd_pasien($kd);
	...
	public function insert(){
		...
		$id_umb 		= $this->M_umb_gigi->create_no_transaction();	
	...
	function insert_data($id_umb,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poli_umb,$hasil,$terapi,$id_poli,$status,$date){
		$data_umb = array(
						...
						'ID_POLI_GIGI'  => $id_poli,
						'Status' 		    => 0,
						'ID_USER' 			=> 5);

		$this->M_umb_gigi->m_insert_data($data_umb);
	}
	...
	function update_data($id_umb,$kd_pasien,$nama,$jenis,$umur,$alamat,$bpjs,$polrim,$petugas,$yth,$poli_umb,$hasil,$terapi,$id_poli,$status,$date){
		...
		$this->M_umb_gigi->m_update_data($data_umb,$id_umb);
	}
