...
class M_daftar_rb extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_pendaftaran_rb';
	public $detail 		= 'dt_pendaftaran_rb';
	...
	function get_all(){
		...
		if($TransStatus == ''){
			$this->db->where('gr.STATUS_RAWAT',0);
		}else{
			$this->db->where_in('gr.status',$TransStatus);
		}
	   ...
	   function m_get_list_pasien($kd_pasien,$nama){
		...
		$this->db->where('br.ID_UNIT','U008');
		$this->db->group_by('br.ID_BEROBAT','ASC');
		$query = $this->db->get()->result_array();
    ...
    function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_DAFTAR_RB,6)) as code_max from tb_pendaftaran_rb");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_suspend_data($data_suspend,$no_transaction){
		$this->db->update($this->table,$data_suspend,array('ID_DAFTAR_RB'=>$no_transaction));
	}
		
	function m_update_group($data_group,$id_daftar){
		$this->db->update($this->table,$data_group,array('ID_DAFTAR_RB'=>$id_daftar));
	}

	function delete_detail($id_daftar){
		$this->db->delete($this->detail,array('ID_DAFTAR_RB'=>$id_daftar));
	}
	
	function m_get_list_kamar($id_kamar,$nm_kamar){
		$this->db->select('ob.ID_KAMAR,ob.NAMA_KAMAR,ob.KETERANGAN');
	...
	function m_get_list_partus($id_partus,$partus){
		$this->db->select('ob.ID_PARTUS,ob.PARTUS,ob.KETERANGAN');  
     ...
     function m_get_list_ibu($id_ibu,$p_ibu){
		$this->db->select('ob.ID_PERAWATAN_IBU,ob.PERAWATAN_IBU,ob.KETERANGAN');
	 ...
	 function m_get_list_bayi($id_bayi,$p_bayi){
		$this->db->select('ob.ID_PERAWATAN_BAYI,ob.PERAWATAN_BAYI,ob.KETERANGAN');
	 ...
	 function m_get_list_bersalin($id_bersalin,$bersalin){
		$this->db->select('ob.ID_TDK_BERSALIN,ob.TINDAKAN_BERSALIN,ob.KETERANGAN');
	 ...
	 function m_get_list_gizi($id_gizi,$makan){
		$this->db->select('ob.ID_GIZI_SMBNG,ob.PEMBERIAN_MAKANAN,ob.KETERANGAN');
	...
	function m_get_group($id_daftar_rb){
	...
	function m_get_detail($id_daftar_rb){
		$this->db->where('ID_DAFTAR_RB',$id_daftar_rb);
	...
	function get_group_rb($id_daftar_rb){
		$this->db->select('gr.*,us.level as lev,em.NAMA_LENGKAP as petugas');
		$this->db->from($this->table.' as gr');
		$this->db->join($this->user.' as us','us.id_user=gr.ID_USER','left');
		$this->db->join($this->employe.' as em','em.ID_EMPLOYEE=gr.ID_EMPLOYEE','left');
		$this->db->where('gr.ID_DAFTAR_RB',$id_daftar_rb);
	...
	function count_detail_rb($id_daftar_rb){
