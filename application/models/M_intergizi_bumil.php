...
class M_intergizi_bumil extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_gizi_ibu_hamil';
	...
	function get_all(){
		...
		if($id_gizi != ''){
			$this->db->like('gr.ID_GIZI_IBU_HAMIL',$id_gizi,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_GIZI_IBU_HAMIL','ASC');
	...
	public function kd_pasien($kd){
      ...
      $result = $this->db->get('tb_pem_gizi_dewasa')->row(); 
	...
	function m_get_list_poli(){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_GIZI_IBU_HAMIL,5)) as code_max from tb_gizi_ibu_hamil");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function create_monev(){
		$q = $this->db->query("select MAX(RIGHT(ID_GIZI_IBU_HAMIL,2)) as code_max from tb_gizi_ibu_hamil");
        $code = "";
        if ($q->num_rows() > 0) {
            foreach ($q->result() as $cd) {
                $tmp = ((int) $cd->code_max) + 1;
        ...
        return "MONEV KE " . $code;
	}
	
	function m_insert_data($data_bumil){
		$this->db->insert($this->table,$data_bumil);
	}
	...
	function m_update_data($data_bumil,$id_bumil){
		$this->db->update($this->table,$data_bumil,array('ID_GIZI_IBU_HAMIL'=>$id_bumil));
	}
	...
	function m_get_group($id_bumil){
		$this->db->where('ID_GIZI_IBU_HAMIL',$id_bumil);
