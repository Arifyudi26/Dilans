...
class M_kunjungan_kb extends CI_Model {
	...
	*/
	...
	function get_all(){
		...
		$id_poli			= isset($param['txt_id']) ? $param['txt_id'] : ''; 
		...
		if($id_poli != ''){
			$this->db->like('gr.ID_KUNJUNGAN_KB',$id_poli,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_KUNJUNGAN_KB','ASC');
	...
	public function kd_pasien($kd){
      ...
      $result = $this->db->get('tb_pemeriksaan_kb')->row(); 
    
      return $result; 
       }

     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_KUNJUNGAN_KB,6)) as code_max from tb_kunjungan_kb");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_group($data_pemeriksaan,$id_kkb){
		$this->db->update($this->table,$data_pemeriksaan,array('ID_KUNJUNGAN_KB'=>$id_kkb));
	}
	...
	function m_get_group($id_kkb){
		$this->db->where('ID_KUNJUNGAN_KB',$id_kkb);
