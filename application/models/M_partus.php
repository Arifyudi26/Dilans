...
class M_partus extends CI_Model {
	...
	*/
	...
	function get_all(){
		...
		$id_partus			= isset($param['txt_no']) ? $param['txt_no'] : ''; 
		$partus			= isset($param['txt_desc']) ? $param['txt_desc'] : ''; 
		...
		$this->db->where('gr.ID_PARTUS !=','UNTCSTMR');

		if($id_partus != ''){
			$this->db->like('gr.ID_PARTUS',$id_partus,'RIGHT');
		}

		if($partus != ''){
			$this->db->like('gr.PARTUS',$partus,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_PARTUS','ASC');
	...
	function get_list_partus($start_date,$end_date,$id_partus,$partus){
		$this->db->select('gr.*');
		$this->db->from($this->table.' as gr');
	...
	function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_PARTUS,4)) as code_max from tb_pertlg_partus where ID_PARTUS!='UNTCSTMR'");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function update_data($data,$code){
		$this->db->update($this->table,$data,array('ID_PARTUS'=>$code));
	}

	function delete_data($code){
		$this->db->delete($this->table,array('ID_PARTUS',$code));
	}
