...
class M_pem_gizi_dewasa extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_pem_gizi_dewasa';
	public $menu 		= 'tbl_menu_pemgizi';
	...
	function get_all(){
		...
		if($id_pemeriksaan != ''){
			$this->db->like('gr.ID_PG_DEWASA',$id_pemeriksaan,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_PG_DEWASA','ASC');
	...
	public function kd_pasien($kd){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_PG_DEWASA,5)) as code_max from tb_pem_gizi_dewasa");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_group($data_pemeriksaan,$id_pem){
		$this->db->update($this->table,$data_pemeriksaan,array('ID_PG_DEWASA'=>$id_pem));
	}


	function delete_data($id_pemeriksaan){
		$this->db->delete($this->table,array('ID_PG_ANAK',$id_pemeriksaan));
	}
	
	
	function m_get_group($id_pemeriksaan){
		$this->db->where('ID_PG_DEWASA',$id_pemeriksaan);
