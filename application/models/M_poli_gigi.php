...
class M_poli_gigi extends CI_Model {
	...
	*/
	...
	function get_all(){
		...
		if($id_gigi != ''){
			$this->db->like('gr.ID_POLI_GIGI',$id_gigi,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_POLI_GIGI','ASC');
	...
	function m_get_list_diagnosa($id_diagnosa,$des_icd){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_POLI_GIGI,5)) as code_max from tb_poli_gigi1");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_insert_data($data_gigi){
		$this->db->insert($this->table,$data_gigi);
	}
	...
	function m_update_data($data_gigi,$id_poli_gigi){
		$this->db->update($this->table,$data_gigi,array('ID_POLI_GIGI'=>$id_poli_gigi));
	}
	...
	function m_get_group($id_poli_gigi){
		$this->db->where('ID_POLI_GIGI',$id_poli_gigi);
