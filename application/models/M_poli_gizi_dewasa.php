...
class M_poli_gizi_dewasa extends CI_Model {
	...
	*/
	...
	function get_all(){
		...
		if($id_gizi != ''){
			$this->db->like('gr.ID_GIZI_DEWASA',$id_gizi,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_GIZI_DEWASA','ASC');
	...
	public function kd_pasien($kd){
	  ...
	  function m_get_list_pasien($kd_pasien,$nama){
		...
		$this->db->group_by('ds.ID_PG_DEWASA');
	...
	function m_get_list_diagnosa($id_diagnosa,$des_icd){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_GIZI_DEWASA,5)) as code_max from tb_gizi_dewasa");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_data($data_gizi,$id_gzd){
		$this->db->update($this->table,$data_gizi,array('ID_GIZI_DEWASA'=>$id_gzd));
	}
	...
	function m_get_group($id_gzd){
		$this->db->where('ID_GIZI_DEWASA',$id_gzd);
