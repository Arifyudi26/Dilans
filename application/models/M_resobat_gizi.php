...
class M_resobat_gizi extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_resobat_gizi';
	public $detail 		= 'dt_resobat_gizi';
	public $obat 		= 'tb_obat';
	...
	function get_all(){
		...
		if($id_terapi != ''){
			$this->db->like('gr.ID_RESOB_GIZI',$id_tarapi,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_RESOB_GIZI','ASC');
    ...
    function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_RESOB_GIZI,5)) as code_max from tb_resobat_gizi");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_resep($data_resep,$id_resep){
		$this->db->update($this->table,$data_resep,array('ID_RESOB_GIZI'=>$id_resep));
	}
	...
	function delete_detail($id_resep){
		$this->db->delete($this->detail,array('ID_RESOB_GIZI'=>$id_resep));
	}
	...
	function m_get_group($id_resob_gizi){
		$this->db->where('ID_RESOB_GIZI',$id_resob_gizi);
	...
	function m_get_detail($id_resob_gizi){
		$this->db->where('ID_RESOB_GIZI',$id_resob_gizi);
	...
	function get_group_resep($id_resob_gizi){
		$this->db->select('gr.*,us.level as lev,dk.NAMA_DOKTER as dokter');
		$this->db->from($this->table.' as gr');
		$this->db->join($this->user.' as us','us.id_user=gr.ID_USER','left');
		$this->db->join($this->dokter.' as dk','dk.ID_DOKTER=gr.ID_DOKTER','left');
		$this->db->where('gr.ID_RESOB_GIZI',$id_resob_gizi);
	...
	function count_detail_resep($id_resob_gizi){
