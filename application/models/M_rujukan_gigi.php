...
class M_rujukan_gigi extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_rujukan_gigi';
	public $menu 		= 'tbl_menu_pgigi';
	public $umum 		= 'tb_poli_gigi';
	...
	function get_all(){
		...
		$id_gigi			= isset($param['txt_id']) ? $param['txt_id'] : ''; 
		...
		if($id_gigi != ''){
			$this->db->like('gr.ID_RUJUKAN_GIGI',$id_gigi,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_RUJUKAN_GIGI','ASC');
	...
	public function kd_pasien($kd){
      ...
      $result = $this->db->get('tb_poli_gigi')->row(); 
	...
	function m_get_list_diagnosa($id_diagnosa,$des_icd){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_RUJUKAN_GIGI,5)) as code_max from tb_rujukan_gigi");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_data($data_rujukan,$id_rujukan){
		$this->db->update($this->table,$data_rujukan,array('ID_RUJUKAN_GIGI'=>$id_rujukan));
	}


	function delete_data($id_rujukan){
		$this->db->delete($this->table,array('ID_RUJUKAN_GIGI',$id_rujukan));
	}
	
	
	function m_get_group($id_rujukan){
		$this->db->where('ID_RUJUKAN_GIGI',$id_rujukan);
