...
class M_rujukan_gizi extends CI_Model {
	...
	*/
	...
	function get_all(){
		...
		if($id_gizi != ''){
			$this->db->like('gr.ID_RUJUKAN_GIZI',$id_gizi,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_RUJUKAN_GIZI','ASC');
	...
	function m_get_list_diagnosa($id_diagnosa,$des_icd){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_RUJUKAN_GIZI,5)) as code_max from tb_rujukan_gizi");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_data($data_rujukan,$id_rujukan){
		$this->db->update($this->table,$data_rujukan,array('ID_RUJUKAN_GIZI'=>$id_rujukan));
	}
	...
	function m_get_group($id_rujukan){
		$this->db->where('ID_RUJUKAN_GIZI',$id_rujukan);
