...
class M_rujukan_kb extends CI_Model {
	...
	*/
	...
	function get_all(){
		...
		if($id_kb != ''){
			$this->db->like('gr.ID_RUJUKAN_KB',$id_lansia,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_RUJUKAN_KB','ASC');
	...
	function m_get_list_diagnosa($id_diagnosa,$des_icd){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_RUJUKAN_KB,5)) as code_max from tb_rujukan_kb");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_data($data,$id_rujukan){
		$this->db->update($this->table,$data,array('ID_RUJUKAN_KB'=>$id_rujukan));
	}


	function delete_data($id_rujukan){
		$this->db->delete($this->table,array('ID_RUJUKAN_KB',$id_rujukan));
	}
	
	
	function m_get_group($id_rujukan){
		$this->db->where('ID_RUJUKAN_KB',$id_rujukan);
