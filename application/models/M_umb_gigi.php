...
class M_umb_gigi extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_umb_rujukan_gigi';
	public $menu 		= 'tbl_menu_pgigi';
	public $umum 		= 'tb_rujukan_gigi';
	...
	function get_all(){
		...
		if($id_gigi != ''){
			$this->db->like('gr.ID_UMB_GIGI',$id_gigi,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_UMB_GIGI','ASC');
	...
	function m_get_list_poli(){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_UMB_GIGI,5)) as code_max from tb_umb_rujukan_gigi");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_data($data_umb,$id_umb){
		$this->db->update($this->table,$data_umb,array('ID_UMB_GIGI'=>$id_umb));
	}
	...
	function m_get_group($id_umb){
		$this->db->where('ID_UMB_GIGI',$id_umb);
