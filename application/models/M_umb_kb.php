...
class M_umb_kb extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_umb_rujukan_kb';
	...
	function get_all(){
		...
		if($id_kb != ''){
			$this->db->like('gr.ID_UMB_KB',$id_kb,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_UMB_KB','ASC');
	...
	function m_get_list_poli(){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_UMB_KB,5)) as code_max from tb_umb_rujukan_kb");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_data($data_umb,$id_umb){
		$this->db->update($this->table,$data_umb,array('ID_UMB_KB'=>$id_umb));
	}
	...
	function m_get_group($id_umb){
		$this->db->where('ID_UMB_KB',$id_umb);
