...
class M_umb_lansia extends CI_Model {
	...
	*/
	...
	public $table 		= 'tb_umb_rujukan_lansia';
	public $menu 		= 'tbl_menu_plansia';
	public $rujukan 		= 'tb_rujukan_lansia';
	...
	function get_all(){
		...
		if($id_lansia != ''){
			$this->db->like('gr.ID_UMB_LANSIA',$id_lansia,'RIGHT');
		}
		...
		$this->db->order_by('gr.ID_UMB_LANSIA','ASC');
	...
	function m_get_list_poli(){
     ...
     function create_no_transaction(){
		$q = $this->db->query("select MAX(RIGHT(ID_UMB_LANSIA,5)) as code_max from tb_umb_rujukan_lansia");
        $code = "";
        if ($q->num_rows() > 0) {
	...
	function m_update_data($data_umb,$id_umb){
		$this->db->update($this->table,$data_umb,array('ID_UMB_LANSIA'=>$id_umb));
	}


	function delete_data($id_umb){

		$this->db->delete($this->table,array('ID_UMB_LANSIA',$id_umb));
	}
	
	
	function m_get_group($id_umb){
		$this->db->where('ID_UMB_LANSIA',$id_umb);
	...
	function get_group_umb($id_umb_umum){
		$this->db->from($this->table.' as gr');
		$this->db->where('gr.ID_UMB_LANSIA',$id_umb_umum);
