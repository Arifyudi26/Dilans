<div class="easyui-layout" style="height:450px;width:100%">
	<div data-options="region:'east',split:true" title="Filter" style="width:20%;height:450px">
		<br>
		<p><input class="easyui-textbox" type="text" name="txt_no" id="txt_no" data-options="prompt:'Id perawatan bayi'" style="height:25px;width:200px"></p>
		<p><input class="easyui-textbox" type="text" name="txt_desc" id="txt_desc" data-options="prompt:'perawatan bayi'" style="height:25px;width:200px"></p>
	...
	<div data-options="region:'center',title:'List Data Perawatan Bayi',iconCls:'icon-ok'" style="width:80%;height:450px">
		<table class="easyui-datagrid" title="" style="width:100%;height:397px" id="grid"
		    ...
		    <thead>
		        <tr>
		            <th data-options="field:'ID_PERAWATAN_BAYI',width:150">Id Perawatan Bayi</th>
                    <th data-options="field:'PERAWATAN_BAYI',width:150">Perawatan Bayi</th>
...
<!-- ================== dialog add perbayi ============================== -->
<div id="dlg_perbayi" class="easyui-dialog" title="Form Perawatan Bayi" style="width:420px;height:300px;padding:10px" closed="true" modal="true"
            data-options="
                iconCls: 'icon-save',
                toolbar: toolbar_2
            ">
    <form id="fm_perbayi" action="<?= base_url().$menu_link->controller;?>/insert" method="post" enctype="multipart/form-data">
	    <table cellpadding="5">
	        <tr>
	            <td width="50" valign="top">Perawatan bayi</td>
	            <td>
	            	<input class="easyui-textbox" name="perbayi" id="perbayi" data-options="required:true" style="height:30px;width:250px"></input>
...
<script type="text/javascript">
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	if(row.status == 0){
	        	$('#status').val(2);
	        	$('#code').val(row.ID_PERAWATAN_BAYI);
	        	$('#perbayi').textbox('setValue',row.PERAWATAN_BAYI);
    ...
    var toolbar_2 = [{
            ...
            iconCls:'icon-reload',
            handler:function(){
               $('#fm_perbayi').form('clear');
            }
        },'-',{
        	text:'Cancel',
            iconCls:'icon-cancel',
            handler:function(){
               $('#dlg_perbayi').dialog('close');
            }
        }];
        get_privillage('<?=$menu_link->MenuID;?>','dlg_<?=$menu_link->controller;?>','d');
		 ...
		 function print_data(){
		    ...
		    var id_bayi      = $('#txt_no').textbox('getValue');
		    var perbayi   	= $('#txt_desc').textbox('getValue');
		    ...
		    if($('#chkdate').is(':checked')){
		        ...
		        parent.printPanel('Preview Pertolongan Bayi Report','<?= $menu_link->controller;?>/print_data','v',startDate,endDate,TransStatus,'',id_bayi,'','',perbayi);
		    }else{
		    	var TransStatus = [];

		    	parent.printPanel('Preview Pertolongan Bayi Report','<?= $menu_link->controller;?>/print_data','v2','','',TransStatus,'',id_bayi,'','',perbayi);
		    }
		}
