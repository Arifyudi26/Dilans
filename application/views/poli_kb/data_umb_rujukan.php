	...
	<div data-options="region:'center',title:'List <?=$menu_link->MenuLabel;?>',iconCls:'icon-ok'" style="width:80%;height:450px">
		<table class="easyui-datagrid" title="" style="width:100%;height:397px" id="grid"
		        ...
		        onDblClickRow:function(){
					...
					if(priedit == false){
        				parent.editPanel('Preview <?=$menu_link->MenuLabel;?>','<?=$menu_link->controller;?>/preview',row.ID_UMB_KB);
        			}
				}
			">
		    <thead>
		        <tr>
		            <th data-options="field:'ID_UMB_KB',width:120">Id Umb Rujukan</th>
...
<!-- ================== dialog add barang ============================== -->
<div id="dlg_umb" class="easyui-dialog" title="Form Edit umb rujukan KB" style="width:800px;height:400px;padding:10px" closed="true" modal="true"
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	if(row.TransStatus == 0){
    			suspend_data('<?= $menu_link->controller;?>/suspend_data','grid',row.ID_UMB_KB);   
	        }
        }
    }];
